...
- `summary.contextConsumers` — WHAT filled the context, by concrete target: each tool result
  ...
  message), and `assistant-tool-calls` (arguments it wrote: Edit payloads, Bash commands,
  subagent prompts; the label names the top tools). One more synthetic row, `unexplained`,
  closes the books: peak context minus everything attributed (token-estimate error, tool-result
  wrappers, injected reminders) — an honesty row, not a consumer, and it carries no cost. The
  header fields say the same thing numerically: `peakContext` (the largest per-step context),
  `attributedTokens` (what the rows account for), `unexplainedTokens` (the remainder, 0 when the
  rows already cover the peak — sessions with resets can attribute MORE than the peak), and
  `totalEstTokens` = attributed + unexplained, which the report uses as the denominator for every
  consumer share bar (it used to be the attributed sum alone, so shares are now shares of peak
  context, not of the attributed subtotal). When
