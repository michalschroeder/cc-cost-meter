...
2. Read `summary.compactionWhatIf.best` — if it is not null AND its `estSaving` is material (≥ 2% of `totalCost`, or ≥ $0.10), the BEST card lever is "compact after turn N": name that turn (use its summary from the turns map), its context then, and the estimated saving. This beats generic "/compact more" advice. Below that threshold the compact point is noise — do not build a card on it.
3. Check each computed signal and use ONLY what the data supports:
   ...
   - `summary.idleGaps` + `summary.cacheRebuilds` = idle-then-re-cache cost. The REPORT ALREADY ADDS its own card for `cacheRebuilds` — never write a second cache-expiry card; use the signal only as evidence inside another card.
   ...
   - Turn kinds (only turns with a non-null kind are user turns; ignore null ones): ≥3 "correction" = correction spiral; many "new-task" without a reset = kitchen-sink session.
