...
2. Does a card name the `compactionWhatIf.best` turn and its saving when `best` is not null AND its `estSaving` is material (≥ 2% of `totalCost`, or ≥ $0.10)? If missing, add it as the top "bad" card. If `estSaving` is below that, a card built on it is noise — drop it.
...
5. Is there a cache-expiry / `cacheRebuilds` card? The report renders its own, deterministically — delete the duplicate.
6. Rewrite any `how` a newcomer could not act on: the exact command (`/compact`, `/clear`, `/effort low`, `--model sonnet`), when to type it, and which saving it recovers. One idea per sentence.
