...
Input: the JSON below — an array of turns, each { turnIndex, kind, tools, prompt }. `prompt` is the user's message (may be terse: "do it", "yes", "but why?"). `tools` is [[toolName, count], …] the turn ran.
